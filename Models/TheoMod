...
class SizeModLogspace:
    ...
    def __init__(self, mld, par, sst, dmdt, minP=0., maxP=np.log10(200), numP=3, minZ=5, maxZ=400, numZ=2, numYears=5,
                 ...
                 Pmax=1.1, Kpar=0.1, alpha=0.15*0.432, Mz=0.1, Mp=0.15, Mz2=0.34, immigration=1e-4,
                 Ieff=0.75, beta=0.69, Kp=3, Rem=0.6, k=0.1,
                 mu_alpha=-0.36, mu_beta=10 ** 0.69, Kpo4_alpha=0.53, Kpo4_beta=10 ** -1.5,
                 graz_alpha=-0.4, graz_beta=26, ops_alpha=0.56, ops_beta=0.65, topt_alpha=-0.11, topt_beta=34.5,
                 SizeTol=[0.2, 0.5], graz_Q10=3, sig_T=15, vectorize=True):  # graz_Q10=2,
        self.mld = mld
        ...
        self.immigration = immigration      # exogeneous
        ...
        self.Fluxes0 = np.zeros(self.numP * 5)
        ...
        self.solution = odeint(self.AlloNPsZD, self.init_con, self.t_array, rtol=1.e-10, atol=1.e-10, hmax=1.e16, hmin=1.e-16)
    ...
    def AlloNPsZD(self, y, t):
        ...
        if self._vectorize:
            Pgrowth = self.Allo_mumax(self.Psize_array) * (self.temp_dependence(self.sst[t], self.Allo_Topt(self.Psize_array)) *
                                                           self.Allo_NU(N, self.Psize_array) * self.LightLim(self.par[t], self.mld[t])) * Ps
            ...
            PgrowthTemp = self.temp_dependence(self.sst[t], self.Allo_Topt(self.Psize_array)) * Ps
            PgrowthNut = self.Allo_NU(N, self.Psize_array) * Ps
            ...
            # [dPsdt]
            var[4:4 + self.numP * 1] = (Pgrowth - Pmortality - grossGraz_S - grossGraz_L - self.mixing(t) * Ps) + self.immigration
            ...
            var[4 + self.numP * 4: 4 + self.numP * 5] = PgrowthTemp
            var[4 + self.numP * 5: 4 + self.numP * 6] = PgrowthNut
