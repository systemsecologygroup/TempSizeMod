...
class SizeModLogspace:
    """
    Size-based model under the assumption of allometric relationships to investigate interactions of nutrient uptake and grazing.
    -------
    ...
    def __init__(self, mld, par, sst, dmdt, N0=1., minP=0., maxP=2., numP=3, minZ=5, maxZ=400, numZ=2, numYears=5,
                 Zsmall0=0.01, Zlarge0=0.01, D0=0.01,
                 Pmax=1.1, Kpar=0.1, alpha=0.15, Mz=0.1, Mp=0.2, Mz2=0.34,
                 Ieff=0.75, beta=0.69, Kp=3, Rem=0.6, k=0.1,
                 mu_alpha=-0.36, mu_beta=10 ** 0.69, Kpo4_alpha=0.53, Kpo4_beta=10 ** -1.5,
        ...
        self.Ps0 = np.repeat(0.01, self.numP, axis=0)  # Initial P concentration (µmol P*L^-1)
        ...
        self.Kp = Kp  # Phytoplankton half-saturation constant (µmol*L^-1)
    ...
    def AlloNPsZD(self, y, t):
        ...
        for i in range(len(Ps)):
            ...
            # [dNdt]
            var[0] = (var[0]
                      - self.Allo_mumax(Psize) * (self.temp(self.sst[t]) * self.Allo_NU(N, Psize) * self.LightLim(self.par[t], self.mld[t])) * P
                      + (self.beta * (1. - self.Ieff) * self.AlloGraz(Psize, self.Zsize_array[0], P, self.SizeTol[0], Graz_denomZsmall)) * Zsmall
                      # Excretion from grazing(Zsmall))
                      + (self.beta * (1. - self.Ieff) * self.AlloGraz(Psize, self.Zsize_array[1], P, self.SizeTol[1], Graz_denomZlarge)) * Zlarge)
            ...
            # [dDdt]
            var[3] = (var[3]
                      + ((1. - self.beta) * self.AlloGraz(Psize, self.Zsize_array[0], P, self.SizeTol[0], Graz_denomZsmall)) * Zsmall  # SloppyfeedP - Zsmall
                      + ((1. - self.beta) * self.AlloGraz(Psize, self.Zsize_array[1], P, self.SizeTol[1], Graz_denomZlarge)) * Zlarge  # SloppyfeedP - Zlarge
                      + self.Mp * P)

            # [dPsdt]
            var[4 + i] = (var[4 + i]
                          + self.Allo_mumax(Psize) * (self.temp(self.sst[t]) * self.Allo_NU(N, Psize) * self.LightLim(self.par[t], self.mld[t])) * P  # Uptk
                          - self.Mp * P  # MortalityP
                          - self.AlloGraz(Psize, self.Zsize_array[0], P, self.SizeTol[0], Graz_denomZsmall) * Zsmall  # GrazingP from Zsmall
                          - self.AlloGraz(Psize, self.Zsize_array[1], P, self.SizeTol[1], Graz_denomZlarge) * Zlarge  # GrazingP from Zlarge
